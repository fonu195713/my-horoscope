...
class Bazi {
  ...
  static void _setTenLords() {
    ...
    tenLords = lordsTable[
        ['甲', '乙', '丙', '丁', '戊', '己', '庚', '辛', '壬', '癸'].indexOf(mainWord)];
  }
  ...
  static Future<void> _countLuckStart() async {
    ...
    solarTermsData = List.from(solarTermsData)
      ..addAll(cycle['y${solarTime.year - 1}']);
    solarTermsData = List.from(solarTermsData)
      ..addAll(cycle['y${solarTime.year + 0}']);
    solarTermsData = List.from(solarTermsData)
      ..addAll(cycle['y${solarTime.year + 1}']);
    ...
    for (int i = 0; i < solarTermsData.length; i++) {
      if (!toDateTime(solarTermsData[i]).isAfter(solarTime) &&
          toDateTime(solarTermsData[i + 1]).isAfter(solarTime)) {
